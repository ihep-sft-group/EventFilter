...
void HltConfigSvc::buildSequenceList(const DOMDocument* xmlDoc, bool first)
{
    if ( first ) {
        first=false;
...
void HltConfigSvc::buildSignatureList(const DOMDocument* xmlDoc, bool first)
{
    if ( first ) {
        first=false;
            ...
            {   
                ...
                if( info_currentNode->getNodeType() && info_currentNode->getNodeType() == DOMNode::ELEMENT_NODE ) // is element
                {
                    ...
                    if (XMLCF(XMLTF(info_currentNode->getNodeName()),"CRITERIA") == 0) {
                       ...
                       //HltCriteria input Vector
                       m_HltCriteria->setBehaviour(behaviour);
                       m_HltCriteria->setScope(getParent()); 
                       //每个sig下的criteria
...
void HltConfigSvc::makeTableVector()
{
  for (unsigned int i = 0; i < m_SequenceVec.size(); i++) {
...
const std::vector<std::pair<Signature*, Sequence*> >& HltConfigSvc::getTablesVector() const
{
