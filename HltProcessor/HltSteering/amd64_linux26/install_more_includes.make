...
#cmt_local_tagfile_install_more_includes = $(HltSteering_tag)_install_more_includes.make
...
#cmt_local_tagfile_install_more_includes = $(HltSteering_tag).make
...
#cmt_final_setup_install_more_includes = $(bin)HltSteering_install_more_includessetup.make
...
#cmt_final_setup_install_more_includes = $(bin)HltSteeringsetup.make
cmt_local_install_more_includes_makefile = $(bin)install_more_includes.make
...
$(install_include_dir) ::
	@if test "$(installarea)" = ""; then \
	  ...
	  if test ! "HltSteering" = ""; then \
	    if test -d ../HltSteering; then \
	      echo "Installing files from more ../HltSteering to $(install_include_dir)"; \
	      if test ! -d $(install_include_dir) ; then mkdir -p $(install_include_dir); fi; \
	      if test ! -d $(install_include_dir)/HltSteering; then mkdir -p $(install_include_dir)/HltSteering; fi;\
	        $(install_command) ../HltSteering $(install_include_dir);\
	    else \
	      echo "No more HltSteering include directory"; \
	    fi; \
          fi; \
	fi
...
install_more_includesuninstall ::
	@if test "$(installarea)" = ""; then \
	  ...
	  echo "Uninstalling files from $(install_include_dir)/HltSteering"; \
	  $(uninstall_command) "$(install_include_dir)/HltSteering/*" ; \
	  if test -f $(install_include_dir)/HltSteering.cmtref ; then \
	    echo "Removing $(install_include_dir)/HltSteering.cmtref"; \
	    eval rm -f $(install_include_dir)/HltSteering.cmtref ; \
	  fi; \
	fi
