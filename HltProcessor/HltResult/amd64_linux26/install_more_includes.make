...
#cmt_local_tagfile_install_more_includes = $(HltResult_tag)_install_more_includes.make
...
#cmt_local_tagfile_install_more_includes = $(HltResult_tag).make
...
#cmt_final_setup_install_more_includes = $(bin)HltResult_install_more_includessetup.make
...
#cmt_final_setup_install_more_includes = $(bin)HltResultsetup.make
cmt_local_install_more_includes_makefile = $(bin)install_more_includes.make
...
$(install_include_dir) ::
	@if test "$(installarea)" = ""; then \
	  ...
	  if test ! "HltResult" = ""; then \
	    if test -d ../HltResult; then \
	      echo "Installing files from more ../HltResult to $(install_include_dir)"; \
	      if test ! -d $(install_include_dir) ; then mkdir -p $(install_include_dir); fi; \
	      if test ! -d $(install_include_dir)/HltResult; then mkdir -p $(install_include_dir)/HltResult; fi;\
	        $(install_command) ../HltResult $(install_include_dir);\
	    else \
	      echo "No more HltResult include directory"; \
	    fi; \
          fi; \
	fi
...
install_more_includesuninstall ::
	@if test "$(installarea)" = ""; then \
	  ...
	  echo "Uninstalling files from $(install_include_dir)/HltResult"; \
	  $(uninstall_command) "$(install_include_dir)/HltResult/*" ; \
	  if test -f $(install_include_dir)/HltResult.cmtref ; then \
	    echo "Removing $(install_include_dir)/HltResult.cmtref"; \
	    eval rm -f $(install_include_dir)/HltResult.cmtref ; \
	  fi; \
	fi
