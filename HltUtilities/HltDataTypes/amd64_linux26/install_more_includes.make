...
#cmt_local_tagfile_install_more_includes = $(HltUtilities_tag)_install_more_includes.make
...
#cmt_local_tagfile_install_more_includes = $(HltUtilities_tag).make
...
#cmt_final_setup_install_more_includes = $(bin)HltUtilities_install_more_includessetup.make
...
#cmt_final_setup_install_more_includes = $(bin)HltUtilitiessetup.make
cmt_local_install_more_includes_makefile = $(bin)install_more_includes.make
...
$(install_include_dir) ::
	@if test "$(installarea)" = ""; then \
	  ...
	  if test ! "HltDataTypes" = ""; then \
	    if test -d ../HltDataTypes; then \
	      echo "Installing files from more ../HltDataTypes to $(install_include_dir)"; \
	      if test ! -d $(install_include_dir) ; then mkdir -p $(install_include_dir); fi; \
	      if test ! -d $(install_include_dir)/HltDataTypes; then mkdir -p $(install_include_dir)/HltDataTypes; fi;\
	        $(install_command) ../HltDataTypes $(install_include_dir);\
	    else \
	      echo "No more HltDataTypes include directory"; \
	    fi; \
          fi; \
	fi
...
install_more_includesuninstall ::
	@if test "$(installarea)" = ""; then \
	  ...
	  echo "Uninstalling files from $(install_include_dir)/HltDataTypes"; \
	  $(uninstall_command) "$(install_include_dir)/HltDataTypes/*" ; \
	  if test -f $(install_include_dir)/HltDataTypes.cmtref ; then \
	    echo "Removing $(install_include_dir)/HltDataTypes.cmtref"; \
	    eval rm -f $(install_include_dir)/HltDataTypes.cmtref ; \
	  fi; \
	fi
