...
#
#   Generated Tue Apr  7 10:31:11 2020  by liuyan
#
#====================================
...
#cmt_local_tagfile_install_more_includes = $(HltSteerData_tag)_install_more_includes.make
...
#cmt_local_tagfile_install_more_includes = $(HltSteerData_tag).make
...
#cmt_final_setup_install_more_includes = $(bin)HltSteerData_install_more_includessetup.make
...
#cmt_final_setup_install_more_includes = $(bin)HltSteerDatasetup.make
cmt_local_install_more_includes_makefile = $(bin)install_more_includes.make
...
$(install_include_dir) ::
	@if test "$(installarea)" = ""; then \
	  ...
	  if test ! "HltSteerData" = ""; then \
	    if test -d ../HltSteerData; then \
	      echo "Installing files from more ../HltSteerData to $(install_include_dir)"; \
	      if test ! -d $(install_include_dir) ; then mkdir -p $(install_include_dir); fi; \
	      if test ! -d $(install_include_dir)/HltSteerData; then mkdir -p $(install_include_dir)/HltSteerData; fi;\
	        $(install_command) ../HltSteerData $(install_include_dir);\
	    else \
	      echo "No more HltSteerData include directory"; \
	    fi; \
          fi; \
	fi
...
install_more_includesuninstall ::
	@if test "$(installarea)" = ""; then \
	  ...
	  echo "Uninstalling files from $(install_include_dir)/HltSteerData"; \
	  $(uninstall_command) "$(install_include_dir)/HltSteerData/*" ; \
	  if test -f $(install_include_dir)/HltSteerData.cmtref ; then \
	    echo "Removing $(install_include_dir)/HltSteerData.cmtref"; \
	    eval rm -f $(install_include_dir)/HltSteerData.cmtref ; \
	  fi; \
	fi
