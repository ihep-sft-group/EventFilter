...
#cmt_local_tagfile_install_more_includes = $(HltAlgorithms_tag)_install_more_includes.make
...
#cmt_local_tagfile_install_more_includes = $(HltAlgorithms_tag).make
...
#cmt_final_setup_install_more_includes = $(bin)HltAlgorithms_install_more_includessetup.make
...
#cmt_final_setup_install_more_includes = $(bin)HltAlgorithmssetup.make
cmt_local_install_more_includes_makefile = $(bin)install_more_includes.make
...
$(install_include_dir) ::
	@if test "$(installarea)" = ""; then \
	  ...
	  if test ! "HltAlgorithms" = ""; then \
	    if test -d ../HltAlgorithms; then \
	      echo "Installing files from more ../HltAlgorithms to $(install_include_dir)"; \
	      if test ! -d $(install_include_dir) ; then mkdir -p $(install_include_dir); fi; \
	      if test ! -d $(install_include_dir)/HltAlgorithms; then mkdir -p $(install_include_dir)/HltAlgorithms; fi;\
	        $(install_command) ../HltAlgorithms $(install_include_dir);\
	    else \
	      echo "No more HltAlgorithms include directory"; \
	    fi; \
          fi; \
	fi
...
install_more_includesuninstall ::
	@if test "$(installarea)" = ""; then \
	  ...
	  echo "Uninstalling files from $(install_include_dir)/HltAlgorithms"; \
	  $(uninstall_command) "$(install_include_dir)/HltAlgorithms/*" ; \
	  if test -f $(install_include_dir)/HltAlgorithms.cmtref ; then \
	    echo "Removing $(install_include_dir)/HltAlgorithms.cmtref"; \
	    eval rm -f $(install_include_dir)/HltAlgorithms.cmtref ; \
	  fi; \
	fi
